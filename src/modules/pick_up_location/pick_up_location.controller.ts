...
export class PickUpLocationsController {
	...
	findOne(@Param('id') id: string): Promise<PickUpLocation> {
		return this.pickUpLocationsService.findOne(+id);
	}

	@Put(':id')
	@ApiOperation({ summary: 'Actualizar lugar de recogida' })
	update(@Param('id') id: string, @Body() updatePickUpLocationDto: PickUpLocationUpdateDto): Promise<PickUpLocation> {
	...
	changeStatus(@Param('id') id: string, @Body('status') status: boolean): Promise<PickUpLocation> {
